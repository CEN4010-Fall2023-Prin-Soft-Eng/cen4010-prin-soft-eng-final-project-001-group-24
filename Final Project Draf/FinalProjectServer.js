...
// Initialize Parse
Parse.initialize(
  "qmiVcBHkyOi90FYFNs6r7e4J5beskXYTkqe85Qqm",
  "zgAXSRve1aW88Ck7dfO06emiorlN5KXmOrtYFuho"
);
...
app.post("/login", (req, res) => {
  ...
  Parse.User.logIn(username, password)
    .then((user) => {
      ...
      res
        .status(200)
        .json({ message: "OK. Redirecting...", redirectTo: "Navigation.html" });
    })
    .catch((error) => {
...
app.post("/signup", (req, res) => {
  ...
  user
    ...
    .then((user) => {
      ...
      res
        .status(200)
        .json({ message: "OK. Redirecting...", redirectTo: "Home.html" });
    })
    .catch((error) => {
    ...
    if (!checkTourSetupExists(identifier)) {
      fs.writeFile(`historical_tours/${identifier}.json`, str, (err) => {
        if (err) {
          ...
          return res
            .status(201)
            .send({ identifier, message: "Tour setup created successfully" });
        }
      });
    } else {
      return res
        .status(200)
        .send({ identifier, message: "Tour setup already exists" });
    }
  });
});
